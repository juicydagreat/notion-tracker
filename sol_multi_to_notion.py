...
TITLE_PROP_PERWALLET = os.getenv("TITLE_PROP_PERWALLET", "Name").strip()     # Title prop in Per-Wallet DB (e.g. Wallet)
TOTAL_TITLE_PROP = os.getenv("TOTAL_TITLE_PROP", "Name").strip()             # Title prop in Daily Total DB
WALLET_ADDR_PROP = os.getenv("WALLET_ADDR_PROP", "Wallet Address").strip()  # Rich text prop holding actual wallet address

# Default = canonical Solana USDC mint
...
def create_daily_total_row(
    db_id: str, date_iso: str,
    sol_total_end: float, sol_total_delta: Optional[float],
):
    props: Dict[str, Any] = {
...
def _sum_mint_token_accounts(wallet: str, mint: str, program_id: str) -> float:
    payload = {
        "jsonrpc": "2.0",
        "id": 1,
        ...
        "params": [
            wallet,
            {"mint": mint},
            {"encoding": "jsonParsed", "commitment": "finalized", "programId": program_id}
        ]
    }
    out = rpc_post(payload)
    accounts = ((out.get("result") or {}).get("value") or [])
...
def rpc_get_token_balance(wallet: str, mint: str) -> float:
    # Query both token programs and add them
    a = _sum_mint_token_accounts(wallet, mint, TOKEN_PROGRAM_ID)
    b = _sum_mint_token_accounts(wallet, mint, TOKEN_2022_PROGRAM_ID)
    return float(a + b)
...
def main():
    ...
    for w in WALLETS:
        ...
        usdc_end = rpc_get_token_balance(w, USDC_MINT)
    ...
    # Log a hard hint if USDC is zero
